...
class Ship:
    def __init__(self, length_ship, bow_ship, direction_ship):
        self.length_ship = length_ship
        self.bow_ship = bow_ship
        self.direction_ship = direction_ship
        self.lives_ship = length_ship

    @property
    def dots(self):
        ship_dots = []
        for i in range(self.length_ship):
            dot_x = self.bow_ship.x
            dot_y = self.bow_ship.y
            if self.direction_ship == 0:
                dot_x += i
            elif self.direction_ship == 1:
                dot_x -= i
            elif self.direction_ship == 2:
                dot_y += i
            elif self.direction_ship == 3:
                dot_y -= i
...
class Board:
    ...
    def __str__(self):
        output_field = ""
        output_field += '  | 1 | 2 | 3 | 4 | 5 | 6 |'
        for i, row in enumerate(self.field):
            output_field += f"\n{i + 1} | " + " | ".join(row) + " |"

        if self.hid:
            output_field = output_field.replace("■", "O")
        return output_field
...
class Game:
    ...
    def try_board(self):
        length_ship = [3, 2, 2, 1, 1, 1, 1]
        board = Board(size=self.size)
        attempts = 0
        for l in length_ship:
            while True:
                attempts += 1
                if attempts > 2000:
                    return None
